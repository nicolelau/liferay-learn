...
* **Impl** modules provide (implement) capabilities using concrete Java classes.
* **Client** modules consume API-defined capabilities. The run time framework injects clients with concrete API implementations.

Here's what you'll do with the example:

1. Launch the Example
1. Examine the API
1. Examine the Implementation (Service)
1. Examine the Client (Consumer)
...
1. Build the example modules.
    ...
    Each module JAR file is generated to that module's `build/libs` folder.
     ...
     ```
     j1h1-api/build/libs/com.liferay.docs.j1h1.greeting-1.0.0.jar
...
1. Deploy the example module JARs.
    ...
    ```bash
    docker cp j1h1-api/build/libs/com.liferay.docs.j1h1.greeting-1.0.0.jar $(docker ps -lq):/opt/liferay/deploy
    ```
...
1. Confirm the deployments in the Docker container console.
    ...
    ```
    STARTED com.liferay.docs.j1h1.greeting_1.0.0
...
└── src
    └── main
        └── java
            └── `com/liferay/docs/j1h1/greeting`
                └── `Greeting.java`
```
...
Bundle-SymbolicName: com.liferay.docs.j1h1.greeting
Bundle-Version: 1.0.0
Export-Package: com.liferay.docs.j1h1.greeting
```

The module's name is *j1h1 Greeting API*. Its symbolic name---a name that ensures uniqueness---is `com.liferay.docs.j1h1.greeting`. Its semantic version is declared next, and its package is *exported*, which means it's made available to other modules. This module's package is just an API other modules can implement.
...
### Examine the API Implementation (Impl)
